...
def old2new(specfile, args):
    ...
    if len(spec.packages) == 1:
        ...
        if family not in exdata['fontconfig']:
            raise TypeError(
                m().error('No fontconfig file for').message(family))
        # single template
        data = {
            'version':
            spec.version,
            ...
            if 'docs' in exdata else '%{nil}',
            'fonts':
            ' '.join([s.name for s in exdata['fonts']]),
            'foundry':
            exdata['foundry'],
            'family':
            family,
        ...
        for k, v in fr.group(exdata['fontinfo']).items():
            ...
            if not summary:
                m([': ']).info(k).warning(
                    'Unable to guess the existing package name. some information may be missing in the spec file'
                ).out()
...
def main():
    ...
    m([': ', ' ']).warning('Note').message(
        'You have to review the result. this doesn\'t guarantee that the generated spec file can be necessarily built properly.'
    ).out()
